...
class EcommerceGoogleAnaltyics extends Extension
{
    ...
    public static $noskuprefix = 'Product-'; //prefix used when no internal item is given to a product.
...
JS;
    ...
    protected function getSKU($item)
    {
        ...
        if ($item->ProductVariationID) {
            ...
            if (!$sku) {
                $sku = self::$noskuprefix.$item->ProductVariation()->Product()->ID;
            }
        } else {
            $sku = $item->Product()->InternalItemID;
            if (!$sku) {
                $sku = self::$noskuprefix.$item->Product()->ID;
            }
        }
        return $sku;
    }
